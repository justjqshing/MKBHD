  ...
  return (
    <div className="w-[screen] h-[screen] max-sm:p-0 p-6 z-50 bg-zinc-300 dark:bg-zinc-900  overflow-hidden mt-24" >
        <div className='flex justify-center mb-12'>
            <h1 className='text-4xl -skew-x-6 text-white font-poppins font-semibold'>All Products</h1>
        </div>
     ...
     {products.map((item, index) => (
        ...
        <div className='min-w-[150px] min-h-[280px] 2xl:w-[20rem] athic:h-[22vw] max-sm:h-[50vh] thic:h-[25vh] max-sm:w-[40vw] max-lg:h-[24rem] 2xl:h-[28rem] 3xl:w-[24vw] 3xl:h-[26vw] max-lg:w-[20rem] xl:h-[29rem] max-2sm:h-[60vw]  h-[39vw] w-[38rem] dark:bg-zinc-800 max-sm:-ml-4 bg-white mt-14 relative overflow-hidden'>
          <div className='w-full flex flex-col justify-center items-center max-sm:mt-5 mt-5'>
            <img className=' w-[20rem] athic:w-[30em] thic:w-[13vh] 3xl:w-[18vw] max-sm:w-[9rem] max-lg:w-[14rem] 2xl:w-[17rem] max-md:w-[12rem] xl:w-[15rem]' src={item.img}></img>
            <div className='xl:w-[15rem] thic:w-[13vh] athic:w-[30em] max-sm:w-[9rem] w-[20rem] max-md:w-[12rem] 2xl:w-[17rem] 3xl:w-[18vw] max-lg:w-[14rem] flex flex-col mt-4 max-sm:mt-2 flex-nowrap gap-4 justify-start'>
             <h1 className={`dark:text-white  font-poppins ${item.name.length > 40 ? 'max-sm:text-[0.7em]' : 'max-sm:text-[1em]'}  max-md:text-[0.9em] max-lg:text-[0.9em] 2xl:text-[1vw] text-[1.2em] thic:text-[1.2vh] font-bold lg:-skew-x-12 uppercase`}>{item.name}</h1>
        ...
        <FontAwesomeIcon  icon={faPlus} className={` -bottom-14 absolute w-[1.5em] max-sm:w-[0.9em]  h-[1.5em] dark:text-white cursor-pointer p-4 hover:rotate-180 duration-500 ease-in-out ${index > (products.length - 5) ? 'flex' : 'hidden'} 
